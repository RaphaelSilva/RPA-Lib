...
from jobs import NavigateJob, SelectElementJob, TypeInputMaskJob
from jobs import ButtonClickJob, LookupBy, SwitchFrameSelectJob
from jobs import ListJob, LookupStock
...
class Clear(Task):
    ...
    def Login(self) -> Sequence:
        return Sequence([
            ...
            SelectElementJob(
                '//div[@class="form-group cont_inputs \
                    filledIdentificationNumber"]//\
                        input[@id="identificationNumber"]'),
            TypeInputMaskJob(self._user.login),
            SelectElementJob(
                '//div[@class="form-group cont_inputs fifty sdate filledDob"]\
                    //input[@id="dob"]'),
            TypeInputMaskJob(self._t_date),
            SelectElementJob(
                '//div[@class="form-group cont_inputs fifty field \
                    filledPassword"]//*[@id="password"]'),
    ...
    def ListWallet(self, wallet_name: str) -> Sequence:
        return Sequence([
            ...
            SwitchFrameSelectJob('content-page',
                                 '//div[@class="tabs--items"]/ul').apply(
                LookupBy(wallet_name, lambda el:
                         el.click() if el.get_attribute('class') != 'active'
                         else print('no click but is active'))
            ),
